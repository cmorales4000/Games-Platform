...
var mensajeConfirmacion = function (mensaje, funcion) {
    ...
    contenido += '</button>';
    contenido += '<button type="button" class="btn blue" onclick="' + funcion + ';$(\'#' + capa + '\').modal(\'hide\');">';
    contenido += '<span class="button-content">Aceptar</span>';
    contenido += '</button>';
    contenido += '</div>';
    $content.append(contenido);
...
var cargarCkeditor = function (identificador, alto, empresa) {
    CKEDITOR.replace(identificador, {
        ...
        filebrowserBrowseUrl: '/FileManager/Explorer.aspx?id=/Files' + (empresa == '' ? '' : '/' + empresa + '/Files')
    });
}
...
var configUploadifyGeneral = function (jInputs) {
    $.each(jInputs, function (i, item) {
        ...
        $('#' + item.id).uploadify({
            ...
            'fileTypeDesc': item.fileDesc,
            'fileTypeExts': item.fileExt,
            ...
            'onUploadSuccess': function (file, data, response) {
                data = eval('(' + data + ')');
                if (data.rta == 'OK') {
                    if ((data.filetype == 'png') || (data.filetype == 'jpg') || (data.filetype == 'gif')) {
                        $('#' + item.divRender).html('<img src="' + data.dirUpload.replace("../","/") + data.filename + '" style="max-heigth:70px; max-width:200px;border:5px solid #ffffff;" />');
                        $('#' + item.hiddenInput).val(data.filename);
                        generarPostcard();
                    }
                    else {
                        $('#' + item.divRender).html('Archivo: <b>' + data.filenameOri + '</b> subido correctamente');
                        $('#' + item.hiddenInput).val(data.filename);
                    }
                    $('#' + item.divRender).append('<a href="javascript:;" class="badge badge-danger" style="position:absolute; left:15px;" onclick="$(\'#' + item.divRender + '\').html(\'\');$(\'#' + item.hiddenInput + '\').val(\'\');"><b>X</b></a>');
                } else if (data.rta == 'NOT') {
                    mensajeAlerta('Se presentaron incovenientes al momento de subir el archivo.');
                } else if (data.rta == 'ERR') {
                    mensajeAlerta('Se genero una exepción. Error: ' + data.msgErr);
                }
            },
...
var configUploadifyGeneralMulti = function (jInputs) {
    $.each(jInputs, function (i, item) {
        ...
        $('#' + item.id).uploadify({
            ...
            'formData': { 'fileQueueID': item.fileQueueID },
            'fileSizeLimit': '15360KB',
            'swf': '/assets/global/swf/uploadify.swf',
            'uploader': '/UploadHandler.ashx',
            'height': item.height,
            'width': item.width,
            'onUploadSuccess': function (file, data, response) {
                ...
                if (data.rta == 'OK') {
                    ...
                    var $content = $('<div/>', {
                        ...
                        'class': 'col-md-3 col-sm-4 mix'
                    });
                    var sContent = '';
                    if ((data.filetype == 'png') || (data.filetype == 'jpg') || (data.filetype == 'gif')) {
                        /*Para este caso se debe incluir los plugin del mixitup, y el fancybox version 2*/
                        sContent += '<div class="mix-inner">';
                        sContent += '<img class="img-responsive" src="' + data.dirUpload.replace("../", "/") + data.filename + '" alt="" />';
                        sContent += '<div class="mix-details">';
                        sContent += '<h4>' + data.filenameOri + '</h4>';
                        ...
                        sContent += '</a>';
                        sContent += '<div style="margin-top:57px;"><a class="mix-preview fancybox-button" href="' + data.dirUpload + data.filename + '" data-rel="fancybox-button" title="' + data.filenameOri + '">';
                        ...
                        sContent += 'Archivo: <b>' + data.filenameOri + '</b> subido correctamente';
                        sContent += '<br /><a href="javascript:;" onclick="$(\'#' + capa + '\').remove();">Eliminar</a>';
                    }
                    sContent += '<input type="hidden" id="inputFile_' + fileCount + '"  name="inputFile_' + fileCount + '" value="' + data.filename + '" />';
                    sContent += '<input type="hidden" id="inputFileName_' + fileCount + '"  name="inputFileName_' + fileCount + '" value="' + data.filenameOri + '" />';
